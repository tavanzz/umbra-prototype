...
namespace Umbra.Prototype
{
    public class ShadowSolidifier : MonoBehaviour
    {
        ...
        private bool TryGetProjectedPose(out Vector3 position, out Quaternion rotation)
        {
            ...
            Vector3 directionFromLight = (shadowCaster.position - lightSource.position).normalized;
            Vector3 direction = (directionFromLight + lightSource.forward).normalized;
            if (direction.sqrMagnitude < 0.001f)
            {
                direction = directionFromLight;
            }

            Ray ray = new Ray(shadowCaster.position, direction);
